...
module "eks_blueprints" {
  ...
  managed_node_groups = {
    ...
    mng1 = {
      ...
      instance_types = ["t3.medium"] # changed by anson
      ami_type       = "AL2_x86_64"
...
module "db" {
  ...
  identifier = "${local.name}-${local.airflow_name}" # changed by anson

  engine               = "postgres"
  engine_version       = "14.3"
  family               = "postgres14"
  major_engine_version = "14"
  instance_class       = "db.t3.medium" # changed by anson
...
module "airflow_s3_bucket" {
  ...
  bucket = "${local.name}-airflow-logs-${data.aws_caller_identity.current.account_id}" # changed by anson
  acl    = "private"
...
resource "aws_secretsmanager_secret" "postgres" {
  name                    = "${local.name}-postgres" # changed by anson
...
resource "aws_secretsmanager_secret" "airflow_webserver" {
  name                    = "${local.name}-airflow_webserver_secret_key" # changed by anson
  recovery_window_in_days = 0 # Set to zero for this example to force delete during Terraform destroy
}
